...
function HomeView({ guide, setGuide, beginCreate, currentGuide }: { guide: GuideName; setGuide: (g: GuideName) => void; beginCreate: () => void; currentGuide: (typeof guides)[number] }) {
  return <>
    <section className="relative mx-auto grid min-h-[calc(100svh-4rem)] max-w-7xl items-center gap-3 px-4 py-5 sm:px-6 lg:grid-cols-[1fr_0.9fr] lg:gap-6 lg:py-16">
      <div className="relative z-10 order-2 max-w-2xl lg:order-1">
        <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-primary/35 bg-primary/10 px-3 py-1.5 text-[10px] font-extrabold uppercase text-primary sm:mb-5 sm:text-xs"><Sparkles className="size-3.5" /> A new kind of best friend</div>
        <p className="font-display text-sm font-bold uppercase tracking-[0.22em] text-muted-foreground">Welcome to Spritemals</p>
        <h1 className="mt-2 font-display text-4xl font-bold leading-[0.94] sm:mt-3 sm:text-7xl lg:text-8xl">YOUR PET.<br/><span className="text-primary">REIMAGINED.</span></h1>
        <p className="mt-3 max-w-xl text-sm leading-6 text-muted-foreground sm:mt-5 sm:text-lg sm:leading-7">Transform the pet you love into a magical AI companion, a virtual plushie, and a custom collectible you can bring home.</p>
        <div className="mt-4 flex gap-2 sm:mt-7 sm:gap-3"><Button variant="portal" size="xl" className="min-w-0 flex-1 px-3 sm:flex-none sm:px-7" onClick={beginCreate}><WandSparkles />Create My Spritemal<ArrowRight /></Button><Button variant="glass" size="xl" className="px-4 sm:px-7" aria-label="Meet the Guides" onClick={() => document.getElementById("guides")?.scrollIntoView({ behavior: "smooth" })}><Play /><span className="hidden sm:inline">Meet the Guides</span></Button></div>
        <div className="mt-4 grid max-w-xl grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] items-center gap-1 text-center text-[8px] font-bold uppercase text-muted-foreground sm:mt-8 sm:gap-2 sm:text-xs"><span>Real Pet</span><ArrowRight className="mx-auto size-3 text-primary sm:size-4"/><span>Spritemal</span><ArrowRight className="mx-auto size-3 text-primary sm:size-4"/><span>Virtual Friend</span><ArrowRight className="mx-auto size-3 text-primary sm:size-4"/><span>Made Real</span></div>
      </div>
      <div className="relative order-1 mx-auto h-[27vh] min-h-52 w-full max-w-lg sm:h-[43vh] sm:min-h-80 lg:order-2 lg:h-[70vh]">
        ...
        <div className="glass-panel absolute bottom-1 left-1 z-20 rounded-lg px-3 py-2 sm:bottom-2 sm:left-2 sm:px-4 sm:py-3"><div className={`font-display text-base font-bold sm:text-xl ${currentGuide.tone}`}>{currentGuide.name}</div><div className="text-[8px] font-bold uppercase text-muted-foreground sm:text-[10px]">AI Spritemal Assistant</div></div>
      </div>
    </section>
